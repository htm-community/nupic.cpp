...
inline void EnumerateSequences(typename SM::value_type th, const SM &g,
                               Sequences &sequences, int rowsOrCols = 0,
                               int noSubsequences = 0) {
  using namespace std;
...
*/
template <typename SM>
inline void FindConnectedComponents(typename SM::value_type th, const SM &g,
...
inline void CuthillMcKeeOrdering(const SM &sm, OutputIterator p,
                                 OutputIterator rp) {
  using namespace std;
