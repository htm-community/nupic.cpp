...
void Link::initPropagationDelayBuffer_(size_t propagationDelay,
  ...
  if (!propagationDelay || propagationDelayBuffer_.size() == propagationDelay) {
  ...
  size_t dataElementCount = original.getCount();
  ...
  if (_LINK_DEBUG) {
    ...
    NTA_DEBUG<<    "    <size> " << propagationDelayBuffer_.size() << " </size>\n";
    for (auto buf : propagationDelayBuffer_) {
      NTA_DEBUG << "    " << buf << "\n";
    }
    NTA_DEBUG <<   "  </propagationDelayBuffer>\n";
  }
...
void Link::compute() {
  ...
  if (propagationDelay_) {
    // A Link's delayed queue buffer size should always be number of delays.
