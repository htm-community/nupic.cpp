 ...
 * further details
 */
class ReadBuffer : public IReadBuffer, public NTA_ReadBuffer {
public:
  ReadBuffer(const nupic::Byte *value, Size size, bool copy = true);
  ...
  Int32 readString(Byte *&value,
                   UInt32 &size,
                   Byte *(*fAlloc)(UInt32 size) = nullptr,
                   void (*fDealloc)(Byte *) = nullptr
				   ) const override;

  template <typename T> Int32 readT(T &value) const {
  ...
  template <typename T> Int32 readT(T *value, Size size) const {
    ReadBuffer *r = const_cast<ReadBuffer *>(this);
    try {
...
class ReadBufferIterator : public IReadBufferIterator,
...
public:
  ...
  template <typename T> Int32 writeT(T value, const char *sep = " ") {
    try {
      if (sep && (getSize() > 0))
        *this << ' ';
      *this << value;
  ...
  template <typename T> Int32 writeT(const T *value, Size size) {
    try {
      for (Size i = 0; i < size; ++i) {
        const T &val = value[i];
        write(val);
