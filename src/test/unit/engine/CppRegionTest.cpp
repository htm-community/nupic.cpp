...
TEST(CppRegionTest, testCppLinkingFanIn) {
  ...
  EXPECT_EQ(r2dims.size(), (size_t)2) << " actual dims: " << r2dims.toString();
  EXPECT_EQ(r2dims[0], (size_t)3) << " actual dims: " << r2dims.toString();
  EXPECT_EQ(r2dims[1], (size_t)2) << " actual dims: " << r2dims.toString();
  ...
  for (size_t node = 0; node < 6; node++) {
    ...
    int row = node / 3;
    int col = node - (row * 3);
    EXPECT_EQ(r2NodeInput.size(), (size_t)8);
...
TEST(CppRegionTest, realmain) {
  ...
  EXPECT_NO_THROW(helperCppInputOutputAccess(level1));
...
TEST(DISABLED_CppRegionTest, memLeak) { //FIXME this mem leak test is newly fixed, but catches error -> need to fix code
  ...
  level1->compute();
  helperCppInputOutputAccess(level1);
	//end main
