...
namespace testing
{
  ...
  {
    std::string p = Path::makeAbsolute("TestOutputDir");
    std::string a = p + sep + "A";
    std::string b = p + sep + "B";

    if (Path::exists(p))
      Directory::removeTree(p);
    ASSERT_TRUE(!Path::exists(p));

    Directory::create(a, false, true); // recursive, readonly
    ASSERT_TRUE(Path::exists(a));

    Directory::create(b);
    ASSERT_TRUE(Path::exists(b));
    std::string src(b + sep + "1.txt");
    if (Path::exists(src))
      Path::remove(src);
    ...
    ASSERT_TRUE(!Directory::exists(a + sep + "B"));

    Directory::copyTree(b, a);
    ASSERT_TRUE(Directory::exists(a));

    std::string dest = a + sep + "1.txt";
    ASSERT_TRUE(Path::exists(dest)) << "File was not copied to destination directory.";
    EXPECT_STREQ(Path::read_all(dest).c_str(), "12345") << "Content of dest file not as expected.";
